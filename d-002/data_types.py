...
however since zero points to the first letter on the left then the
last character (i.e. furthest right) starts at index '-1', then '-2'
and so on moving from right to left back through the string.
"""
...
-- often considered equivalent to '1' or '0' respectively (blame binary!).
"""
True
False
...
-- Note: the 'len()' function returns a TypeError if we pass in a number!

Anyway, in order to fix this error we can use another function in Python
to convert (or 'cast') the variable's data-type to a string:
"""
...
"""
It is also possible to cast/convert to a Boolean value using the 'bool()'
...
# However there are some quirks to consider when casting to a 'bool':
# - any number that is NOT zero equates to "True"
print(bool(-1)) # -> "True"
